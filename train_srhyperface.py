...
from ESRGAN.model import RRDBNet
...
sr_model = RRDBNet(3, 3, 64, 23, gc=32).to(device)
sr_ckpt = root / 'ESRGAN' / 'checkpoints' / 'interp_03.pth'
sr_model.load_state_dict(torch.load(sr_ckpt), strict=True)
sr_model.eval()

sr_aflw = SR_AFLW(data_root, 227, sr_model)
